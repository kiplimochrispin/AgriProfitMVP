...
const updateActiveBadge = () => {
    ...
    if (active && !activeUserId())
        setActiveUserId(active.id);
    setText("active-user-badge", active
...
const renderTrend = (labels, values) => {
    ...
    const grid = [0, 0.25, 0.5, 0.75, 1]
        ...
        .join("");
    const dots = points
        .map(([x, y], index) => `<circle cx="${x}" cy="${y}" r="5" fill="#f7f4ec" stroke="#2f6a3e" stroke-width="3"></circle>
...
const renderRecordList = (id, markup) => {
    const container = document.getElementById(id);
    if (container)
        container.innerHTML = markup || `<div class="activity-item">No records yet.</div>`;
};
...
const loadUsers = async () => {
    ...
    updateActiveBadge();
    renderRecordList("users-list", state.users
        .map((u) => `<div class="activity-item"><strong>${u.full_name || u.phone}</strong><br>${u.county} • ${u.soil_type || "soil pending"}${actionButtons("user", u.id)}</div>`)
        .join(""));
};
const loadCropPlans = async () => {
    ...
    renderRecordList("plans-list", state.cropPlans
        .map((p) => `<div class="activity-item"><strong>${p.crop_type}</strong><br>${p.season_year} • ${p.acres} ac${actionButtons("crop-plan", p.id)}</div>`)
        .join(""));
};
const loadInputs = async () => {
    state.inputs = await apiFetch("/inputs/");
    renderRecordList("inputs-list", state.inputs
        .map((i) => `<div class="activity-item"><strong>${i.item_name}</strong><br>${i.category} • KSh ${Number(i.cost_ksh).toLocaleString()}${actionButtons("input", i.id)}</div>`)
        .join(""));
};
const loadHarvests = async () => {
    state.harvests = await apiFetch("/harvests/");
    renderRecordList("harvests-list", state.harvests
...
const postJSON = (url, payload) => apiFetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
});
const patchJSON = (url, payload) => apiFetch(url, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
});
...
document.getElementById("crop-plan-form").addEventListener("submit", async (event) => {
    ...
    form.reset();
    form.elements.namedItem("season_year").value = "2026";
    await refreshAll();
...
document.getElementById("harvest-form").addEventListener("submit", async (event) => {
    ...
    setActiveCropPlanId(String(payload.crop_plan_id));
    form.reset();
    form.elements.namedItem("other_costs_ksh").value = "0";
    await refreshAll();
...
document.addEventListener("click", async (event) => {
    ...
    if (action === "edit") {
        ...
        if (kind === "user") {
            ...
            if (full_name !== null && county !== null)
                await patchJSON(`/users/${id}`, { full_name, county });
        }
            ...
            if (crop_type !== null && acres !== null)
                await patchJSON(`/crop-plans/${id}`, { crop_type, acres: Number(acres) });
        }
        else if (kind === "input") {
            const input = record;
